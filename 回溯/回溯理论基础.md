    ...
    void backtrace(string &s, int index) {
        ...
        for (int right = index; right < n; right++) {
            ...
            if (is_palindrome(s, index, right)) {
                // ===== 选择 =====
                // 当前这段是回文，可以加入路径
                split_str.push_back(s.substr(index, right - index + 1));
...
**核心思路**：
- 使用两个计数器：`left`（左括号数）和`right`（右括号数）
- 左括号：只要`left < n`就可以添加
- 右括号：只有`right < left`时才能添加（保证有效性）
...
**关键点**：
- 两个计数器：`left`和`right`分别记录左右括号数量
- 有效性保证：`right < left`时才添加右括号
- 终止条件：`left == n && right == n`
...
**关键点**：
- 使用`visited`数组：标记已访问位置，避免重复使用
- 四个方向搜索：上下左右四个方向
- 回溯恢复状态：`visited[i][j] = false`
- 找到即返回：找到一条路径就返回true

### 8.4 N皇后

**适用场景**：在N×N棋盘上放置N个皇后，使得它们不能相互攻击

**核心思路**：
- 使用三个数组记录列和对角线的占用情况
- 主对角线：`row + col`为常数
- 副对角线：`row - col + n - 1`为常数（整体平移避免负数）
...
private:
    ...
    void backtracking(int row, 
        ...
        for (int c = 0; c < n; c++) {
            ...
            board[row][c] = 'Q';
            col[c] = true;
            diag1[row + c] = true;              // 主对角线(左下右上)
            diag2[row - c + n - 1] = true;      // 副对角线（整体平移n-1）（右下左上）
...
**关键点**：
- 三个数组记录：列、主对角线、副对角线
- 主对角线：`row + col`为常数（0到2n-2）
- 副对角线：`row - col + n - 1`为常数（整体平移避免负数）
- 冲突检测：`col[c] || diag1[row + c] || diag2[row - c + n - 1]`
...
- **子集类**：子集、子集II、递增子序列
- **排列类**：全排列、全排列II
- **其他类**：电话号码的字母组合、括号生成、单词搜索、N皇后、重新安排行程
